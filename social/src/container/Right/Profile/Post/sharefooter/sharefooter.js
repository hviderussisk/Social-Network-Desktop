...
function ShareFooter(props) {
    ...
    let commentList = props.comment.map((el, i)=><Comment photo={props.photo} deleteComment={props.deleteComment} id={el.id} key={el._id} post={props.post} name={el.name} timePost={el.timePost} info={el.info}></Comment>)
    return (
        <>
            ...
            {!countAddComment ? <></> : <AddCommentContainer photo={props.photo} post={props.post} updateNewPostTextComment={props.updateNewPostTextComment}/>}
        </>
